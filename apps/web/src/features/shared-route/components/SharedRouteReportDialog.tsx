/**
 * 사용 위치: 공유 루트 상세 → 작성자 영역 → 신고
 *
 * 용도:
 * 공유 루트에 맞는 신고 사유를 선택하고 필요한 경우 상세 내용을 입력한다.
 *
 * 구조:
...
import type { SharedRouteReportReason } from "@/generated/graphql";
...
function SharedRouteReportDialog({
  isOpen,
  isSubmitting,
  onClose,
  onSubmit,
  text,
}: SharedRouteReportDialogProps) {
  const [reason, setReason] = useState<SharedRouteReportReason | null>(null);
  const [details, setDetails] = useState("");

  if (!isOpen) return null;

  const options: Array<{ value: SharedRouteReportReason; label: string }> = [
    { value: "INAPPROPRIATE", label: text.inappropriate },
    { value: "MISLEADING", label: text.misleading },
    { value: "SPAM", label: text.spam },
    { value: "HARASSMENT_OR_HATE", label: text.harassmentOrHate },
    { value: "PRIVACY_OR_RIGHTS", label: text.privacyOrRights },
  ...
  return createPortal(
    <div className="center-modal-backdrop-enter fixed inset-0 z-[3600] flex items-end justify-center bg-slate-950/55 px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-6 sm:items-center">
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="shared-route-report-title"
...
export default SharedRouteReportDialog;
